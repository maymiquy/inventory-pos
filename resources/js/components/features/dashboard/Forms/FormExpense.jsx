...
const FormExpense = ({
  ...
  const submit = (e) => {
    ...
    submitFunction(url, {
      ...
      onSuccess: () => {
        toast({
          title: `Successfully ${submitAction === 'create' ? 'Created' : 'Updated'} Expense`,
          description: `The expense has been ${submitAction === 'create' ? 'created' : 'updated'} successfully`,
          variant: 'success',
        });
      },
      onError: () => {
        ...
        toast({
          title: `Failed to ${submitAction === 'create' ? 'Create' : 'Update'} Expense`,
          description:
            errors.error ||
            `Something went wrong trying to ${submitAction === 'create' ? 'create' : 'update'} the expense, please try again`,
          variant: 'destructive',
        });
      },
    });
    ...
    <form onSubmit={submit}>
      <div className="flex w-full flex-col space-y-12">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <Label
              htmlFor="restock_date"
              className="block font-semibold"
            >
              Restock Date
            </Label>
            <DatePicker
              value={data.restock_date}
              onChange={(date) =>
                setData('restock_date', date)
              }
              className="mt-1 block w-full"
            />
            {errors.restock_date && (
